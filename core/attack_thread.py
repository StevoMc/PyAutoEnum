                    ...
                    if not os.path.exists(self.module.output_file):
                        with open(self.module.output_file, "w") as outfile:
                            ...
                            if isinstance(str_ouput, str):
                                ...
                                raise Exception(
                                    f"Output of {self.module.name} not writeable to output file {self.module.output_file}"
                                )
            else:
    ...
    def _run_external_command(self):
        ...
        try:
            with open(self.module.output_file, "w") as outfile:
                ...
                subprocess.call(
                    " ".join(command), stdout=outfile, stderr=outfile, shell=True
                )
        except Exception as e:
            Config.log_error(
                f"Failed to execute external command {self.module.command}: {e}"
            )
            raise
    ...
    def _process_analysis(self):
        ...
        if analyse_func:
            ...
            if self.output:
                ...
                Config.log_warning(
                    f"{self.module.name} did not return usable output for {self.module.analyse_func}"
                )
