...
def print_data(data_win):
    ...
    for key, value_dict in data_unordered.items():
        if key == "0":
            continue
        data[key] = {
            value_dict_key: value_dict[value_dict_key]
            for value_dict_key in custom_order
        }

    headers = ["Ports"] + list(data[next(iter(data))].keys())
    index_width = 5
    column_widths = [index_width] + [
        max(len(str(header)), max(len(str(row[header])) for row in data.values()))
    ...
    for key, value in data.items():
        row_line = (
            str(key).ljust(index_width)
            + " | "
            + " | ".join(
                truncate_value(str(value.get(header, "")), width).ljust(width)
...
def main(stdscr):
    ...
    while True:
        ...
        time.sleep(0.01)
        data_win.addstr(
            f"Modules: Running {AttackThread.running_count}, Finished {AttackThread.finished_count}, Errors {AttackThread.error_count}\n\n"
        )
        print_data(data_win)
